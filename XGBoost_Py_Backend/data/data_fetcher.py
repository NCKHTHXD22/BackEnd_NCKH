...
def fetch_meteo_history(lat, lon, days=11) -> pd.DataFrame:
    ...
    Lấy lịch sử khí tượng (past days) từ Open-Meteo ERA5 archive:
      time | temperature | relative_humidity | pressure | et0 | wind_speed

    Dùng để bổ sung X_past khi inference. "pressure" (surface_pressure) thêm
    vào để khớp đủ 47 feature của LSTM_Py_Backend_v2/data/nwp_fetcher.py
    (RF_Py_Backend/XGBoost_Py_Backend train trên dataset 47-feature của v2,
    không phải 44-feature của v1 — thiếu pressure ở đây sẽ lệch train/serve).
    """
