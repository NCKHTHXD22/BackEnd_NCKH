...
Copy nguyên từ LSTM_Py_Backend/lstm_service/data/rain_matrix_loader.py.
...
Cấu trúc MỚI (sau update_excel_z_qout.py):
  ...
  Col 121+   : Dữ liệu trạm riêng lẻ (bỏ qua — xem TODO trong data/dataset_builder.py
               nếu muốn parse để dùng cho StationRainAttention)
...
# --------------------------------------------------------------------------- #
#  Mưa từng trạm riêng lẻ (Col 121+) — dùng cho StationRainAttention           #
#                                                                               #
#  Đã verify trực tiếp trên file Excel thật (HO_A_VUONG, HO_DAK_MI_4):        #
#    Col 121 + i*24 .. 121 + i*24 + 23  =  24 giờ của trạm thứ i (0-indexed), #
#    theo ĐÚNG thứ tự RESERVOIR_TO_STATIONS[rid] trong config/rain_stations.py#
#    (label "Trạm: <tên>" ở row 0, "(2022+)"/"(2025+)" ở row 2 khớp chính xác #
#    với available_from đã khai báo sẵn trong config).                        #
# --------------------------------------------------------------------------- #

STATION_COL_START = 121
STATION_COL_WIDTH = 24


def _parse_station_rain_monthly(filepath: str, station_keys: list) -> pd.DataFrame:
    """
    Đọc mưa từng trạm từ 1 file Excel tháng.
    Trả về DataFrame: time | <station_key_1> | <station_key_2> | ...
    (NaN nếu trạm đó chưa có dữ liệu ở thời điểm này — vd trạm "(2025+)" trong file 2023).
    """
    try:
        df_raw = pd.read_excel(filepath, header=None, dtype=object)
    except Exception as e:
    ...
    for _, row in df_data.iterrows():
        ...
        for h in range(24):
            ...
            for i, sk in enumerate(station_keys):
                col = STATION_COL_START + i * STATION_COL_WIDTH + h
                if col >= n_cols:
                    rec[sk] = np.nan
                    continue
                try:
                    v = float(row.iloc[col])
                    rec[sk] = v if (not np.isnan(v) and v >= 0) else np.nan
                except Exception:
                    rec[sk] = np.nan
            records.append(rec)
...
def load_station_rain_matrix(rid: int, station_keys: list,
                              start_date=None, end_date=None) -> pd.DataFrame:
    """
    Trả về DataFrame hourly: time | <station_key_1> | <station_key_2> | ...
    station_keys PHẢI đúng thứ tự config/rain_stations.py::RESERVOIR_TO_STATIONS[rid]
    (quyết định vị trí cột trong Excel).
    """
    folder_name = _get_folder_name(rid)
    if not folder_name:
        return pd.DataFrame()
    ...
    for y, m in _months_between(start_date, end_date):
        ...
        if not os.path.exists(fpath):
            continue
        df = _parse_station_rain_monthly(fpath, station_keys)
        if not df.empty:
            all_frames.append(df)
    ...
    result = pd.concat(all_frames, ignore_index=True)
    result = result.sort_values("time").drop_duplicates("time")
    result = result[(result["time"] >= start_date) & (result["time"] <= end_date)]
